...
//importante existir esse import para ser possivel adicionar logs e prints aos relatorios de saida
...
//comando que adiciona log ao relatorio
Cypress.Commands.add("addContext", (context) => {
  cy.once("test:after:run", (test) => addContext({ test }, context));
});

//comando que adicona log e print correspondente ao relatorio
Cypress.Commands.add("logPrint", (log) => {
  cy.screenshot(log);

  const context = `${Cypress.spec.name}\\` + log + ".png";

  cy.once("test:after:run", (test) => addContext({ test }, log));
  cy.once("test:after:run", (test) => addContext({ test }, context));
});

Cypress.Commands.add("getIframe", (iframeSrc) => {
  const getIframeDocument = () => {
    return (
      cy
        .get(`iframe[src="${iframeSrc}"]`)
...
Cypress.Commands.add("resetSenhaPadrão", (username, password) => {});
